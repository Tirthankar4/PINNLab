...
class ParameterEmbedding(nn.Module):
    def __init__(self, activation=None, num_layers=2, num_neurons=32):
        ...
        layers.append(get_activation(activation if activation is not None else PARAM_EMBED_ACTIVATION, nn.Tanh()))
...
class InputEmbedding(nn.Module):
    def __init__(self, activation=None, num_layers=3, num_neurons=64):
        ...
        for i in range(num_layers - 1):
            layers.append(nn.Linear(num_neurons, num_neurons))
            layers.append(get_activation(activation if activation is not None else INPUT_EMBED_ACTIVATION, Sin()))
...
class BasePINN(nn.Module):
    def __init__(self, input_dim, output_dim, num_neurons=96, use_param_embedding=False,
        ...
        layers = []
        layers.append(nn.Linear(input_dim_main, num_neurons))
        layers.append(get_activation(main_pinn_activation if main_pinn_activation is not None 
