...
def plot_function(net,time_array,initial_params, N, velocity = False,isplot =False ,animation=False):
    ...
    if animation:
        ...
        if isinstance(time_array, (int, float)):
            time_array = np.array([time_array])
        elif isinstance(time_array, list):
            time_array = np.array(time_array)
    ...
    #         plt.axhline(rho_theory , color = 'r', linestyle = '--')
            ...
            #plt.savefig(output_folder+'/PINNS_rho_max'+str(lam)+'_'+str(num_of_waves)+'_'+str(tmax)+'.png', dpi=300)'''
...
def rel_misfit(net, time_array, initial_params, N, nu, num_of_waves, rho_1):
    ...
    for time,j in zip(time_array,range(len(time_array))):
        ...
        x,rho,v,phi,n,rho_LT,rho_LT_max,rho_max,v_LT= LAX.lax_solution1D(time,N,nu,lam,num_of_waves,
    ...
    #     axes[2][j].set_ylim(-0.6,0.6)
        ...
        # Show legend only in last column for 4th row
        if j == len(time_array)-1:
            axes[3][j].legend(loc='best',fancybox=False, shadow=False, ncol=3,fontsize = 10)
        # axes[1][0].set_ylabel(r"$\rho_{PN}- \rho_{FD or LT}/(0.5 (\times\rho_{PN}+ \rho_{FD or LT}))$",fontsize = 10)
